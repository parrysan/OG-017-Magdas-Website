...
function ComponentBlock({ name, source = 'os-000', isNew = false, children }: ComponentBlockProps) {
  return (
...
function TokenSwatch({ name, value, type = 'color' }: TokenSwatchProps) {
  if (type === 'color') {
    return (
...
export default function DesignSystemPage({ projectName = 'Project', newComponents = [] }: DesignSystemPageProps) {
  const sampleLinks = [
    { label: 'Home', href: '#' },
    { label: 'About', href: '#' },
    { label: 'Services', href: '#' },
